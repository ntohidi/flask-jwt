...
class UserModel:
    ...
    def verify_password(self, pwd):
        if not check_password_hash(self.password, pwd):
            return False # jsonify(status=403, error="wrong password")
        return True # jsonify(status=200)

    @staticmethod
    def return_all():
        try:
            _users = list(users.find({}, {'_id': 0}))
            return _users
            # return [UserModel(user) for user in _users]
        except Exception as err:
    ...
    def delete_all():
        try:
            ...
            print("[UserModel Delete All Error]: " + str(err))
            return False
